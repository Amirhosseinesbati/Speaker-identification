"""
Vendored ERes2NetV2 speaker embedding architecture (3D-Speaker / Alibaba).

Copied verbatim from the ModelScope distribution
(modelscope.models.audio.sv.ERes2NetV2 + fusion.AFF + pooling_layers.TSTP)
so the encoder is fully self-contained — it needs only torch + torchaudio,
with NO modelscope / 3dspeaker dependency. This is the most robust offline
loading path for the leaderboard server.

Reference: "ERes2NetV2: Towards Better Performance and Robustness on
Short-Duration Speaker Verification" (Chen et al.).
"""
...
class AFF(nn.Module):
    """Attentional Feature Fusion (bottom-up dual-stage feature fusion)."""
...
class TSTP(nn.Module):
    """Temporal statistics pooling (mean + std), used in x-vector."""
...
class ERes2NetV2(nn.Module):
    """ERes2NetV2 backbone (BDFF + BLFF), 192-dim embeddings by default."""
    ...
    def __init__(self,
        ...
        self.pool = _pooling_registry()[pooling_func](
...
def _pooling_registry():
    """Pooling name → class map (only TSTP is needed by ERes2NetV2)."""
    return {"TSTP": TSTP}
