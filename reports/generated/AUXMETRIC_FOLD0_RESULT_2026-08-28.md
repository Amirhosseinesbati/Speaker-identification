...
## ممیزی سکوت و collapse خروجی

ممیزی streaming روی PCM تمام ۱۶۳۲ فایل Fold 0 علت گروه ۱۱تاییِ بردارهای
احتمال دقیقاً یکسان را روشن کرد. ده فایل کاملاً سکوت دیجیتال‌اند
(`peak=0`, `RMS=0`) و فایل یازدهم عملاً سکوت است (`RMS=1.18e-7`، فقط
`1.49e-5` از نمونه‌ها غیرصفر). در کل Fold، ۱۰ فایل سکوت دقیق و ۲۳ فایل با
`RMS < 1e-4` وجود دارد.

این collapse ناشی از fallback خطای decode نیست: هر ۱۶۳۲ WAV بدون خطا خوانده
شد، هیچ فایل byte-identical یا PCM-identical پیدا نشد و طول فایل‌های گروه
از `1.024s` تا `94.891s` متغیر است. بنابراین اطلاعات صوتی این نمونه‌ها واقعاً
تهی یا نزدیک به تهی است و مدل نمی‌تواند هویت Known را از waveform بازیابی کند.
در گروه ۱۱تایی، برچسب‌ها شامل دو OOD و نه Known هستند؛ Aux با خروجی ثابت OOD
دو مورد OOD را درست می‌کند و Control با خروجی ثابت کلاس ۱۴۸ دو Known را درست
می‌کند. این برابری نشان می‌دهد انتخاب fallback از همین Fold فاقد شاهد تعمیم است.

قاعدهٔ کم‌انرژی برای Fold 0 یا submission به‌صورت پس‌نگرانه انتخاب نمی‌شود.
در Fold 1 فقط قواعد ازپیش‌ثبت‌شدهٔ `RMS < 1e-4`، سکوت دقیق، و prior محاسبه‌شده
صرفاً از train همان Fold به‌عنوان diagnostic تأییدی سنجیده خواهند شد. تصمیم
استفاده در inference تنها پس از شاهد Fold مستقل مجاز است.

Artifactهای این ممیزی:

- JSON: SHA-256 برابر
  `5a911df0595fbfced6a912812a24fc0fa77e36ccf65a1f623237425df025e2cd`
- CSV موارد تشخیصی: SHA-256 برابر
  `cf5bd03428bd9b47c46e2ae96d4cc1f756d4d54b8ad0e985430cda3848281f6d`
- ابزار و تست signal-level: commit `b882525`
...
تحلیل مدت‌زمان، identity و signal-collapse روی Worker تکمیل شد. نسخهٔ پایه در
commit `a92550c` و ممیزی PCM در commit `b882525` ثبت شده‌اند؛ تست‌های محدود
نسخهٔ نهایی روی Windows و Worker هر دو `6/6` موفق بودند. هیچ نتیجهٔ Gate
مدت‌زمانی یا کم‌انرژی از Fold 0 برای submission انتخاب نشده است.
