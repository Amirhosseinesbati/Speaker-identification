"""Cross-fit nuisance attribute projection for the locked CAM++ LME-20 rule.

The backend estimates within-speaker covariance from the 446 ground-truth
known speakers in each matching training fold.  It removes a small fixed number
of its highest-variance directions, length-normalises the embeddings, and then
uses the unchanged LME-20 prototypes and decision rule.  A held-out fold never
selects its own projection dimension.
"""
...
def nuisance_projection(
    covariance: np.ndarray, removed_dims: int
) -> tuple[np.ndarray, dict]:
    covariance = np.asarray(covariance, dtype=np.float64)
    if covariance.ndim != 2 or covariance.shape[0] != covariance.shape[1]:
        raise ValueError("Covariance must be square")
    if not 0 < int(removed_dims) < covariance.shape[0]:
        raise ValueError("removed_dims must be between 1 and dimension - 1")
    eigenvalues, eigenvectors = np.linalg.eigh(covariance)
    nuisance = eigenvectors[:, -int(removed_dims):]
    projection = np.eye(covariance.shape[0]) - nuisance @ nuisance.T
    projection = 0.5 * (projection + projection.T)
    total_variance = float(np.maximum(eigenvalues, 0.0).sum())
    removed_variance = float(
        np.maximum(eigenvalues[-int(removed_dims):], 0.0).sum()
    )
    return projection, {
        "removed_dims": int(removed_dims),
        "embedding_dims": int(covariance.shape[0]),
        "eigenvalue_min": float(eigenvalues.min()),
        "eigenvalue_median": float(np.median(eigenvalues)),
        "eigenvalue_max": float(eigenvalues.max()),
        "removed_within_variance_fraction": (
            removed_variance / max(total_variance, 1e-15)
        ),
        "projection_symmetry_error": float(
            np.max(np.abs(projection - projection.T))
        ),
        "projection_idempotence_error": float(
            np.max(np.abs(projection @ projection - projection))
        ),
    }
...
def prepare_fold(
    *, fold: int, artifact: dict[str, np.ndarray], oof: dict
    ...
    diagnostics = {
        ...
        "covariance_groups": int(len(known_groups)),
        "within_group_degrees_of_freedom": int(degrees_of_freedom),
        "removed_dims": {},
    }
    for removed_dims in REMOVED_DIMS:
        projection, projection_diagnostics = nuisance_projection(
            covariance, removed_dims
        )
        transformed_train = l2norm_rows(train @ projection)
        transformed_validation = l2norm_rows(validation @ projection)
        transformed_scores = logmeanexp_group_scores(
            transformed_validation, transformed_train, groups
        )
        candidates[int(removed_dims)] = fixed_lme_predictions(
            head=head, raw_scores=transformed_scores
        )
        diagnostics["removed_dims"][str(removed_dims)] = {
...
def fold_metrics(fold: FoldNap, removed_dims: int) -> tuple[dict, dict, dict]:
    baseline = metric_bundle(fold.labels, fold.baseline_predictions)
    candidate = metric_bundle(
        fold.labels, fold.candidate_predictions[removed_dims]
    )
    return baseline, candidate, metric_delta(candidate, baseline)


def select_removed_dims(
    folds: list[FoldNap], calibration_folds: tuple[int, int]
) -> tuple[int, dict]:
    ranked = []
    for removed_dims in REMOVED_DIMS:
        rows = []
        ...
        ranked.append((rank, int(removed_dims), rows, feasible))
    rank, removed_dims, rows, feasible = max(
...
def aggregate_evaluation(
    folds: list[FoldNap], predictions: list[np.ndarray]
) -> dict:
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "reports" / "generated" / "campp_lme20_nap_crossfit.json",
    )
    ...
    for target in range(NUM_FOLDS):
        ...
        removed_dims, selection = select_removed_dims(
            folds, calibration  # type: ignore[arg-type]
        )
        baseline, candidate, delta = fold_metrics(folds[target], removed_dims)
        selections.append({
            "target_fold": target,
            "parameters": {"removed_dims": removed_dims},
        ...
        predictions.append(folds[target].candidate_predictions[removed_dims])
    ...
    for removed_dims in REMOVED_DIMS:
        evaluation = aggregate_evaluation(
            folds,
            [fold.candidate_predictions[int(removed_dims)] for fold in folds],
        )
    ...
    report = {
        "contract": {
            ...
            "covariance_groups": "train-only 446 ground-truth known speakers",
            "candidate_removed_dims": list(REMOVED_DIMS),
            "lme_beta": LME_BETA,
            "fixed_decision": {
                "alpha": LOCKED_ALPHA,
        ...
        "selections": [
            {
                ...
                "removed_dims": row["parameters"]["removed_dims"],
